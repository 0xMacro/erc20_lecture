...
contract PaymentSplitter {
    ...
    function split(IERC20 token, uint256 amount, address[] calldata _addrs) external payable {
        require(amount > 0, "must send in non-zero amount");

        uint256 numAddrs = _addrs.length;
        uint256 amountToSend = amount / _addrs.length;
        require(amountToSend * numAddrs == amount, "amount must be evenly divisible by the number of input addresses");
